...
def main():
    ...
    labels = ["KGE", "BIAS", "VARIANCE", "CORRELATION"]
    datasets = []
    for _ in labels:
        data = 1.0 - np.abs(rng.normal(0.0, 1.2, 100))
        datasets.append((
            data,
            data*0.75,
            np.where(data<0.75, data+0.25, data)
        ))
    hgrid = HistogramGrid(datasets, labels, 2)
    ...
    def update_button(event):
        ...
        new_datasets = []
        for _ in labels:
            s = rng.integers(10, 1000, 1)
            data = rng.uniform(-1.01, 1.01, s[0])
            new_datasets.append((
                data,
                data*0.95,
                np.where(data<0.95, data+0.05, data)
            ))
        hgrid.update_data(new_datasets)
    button = pn.widgets.Button(name="Click me")
    pn.bind(update_button, button, watch=True)
    pn.serve(pn.Column(hgrid.servable(), button))
